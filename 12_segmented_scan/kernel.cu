...
LaunchMetrics launch_segmented_scan(const float* a, const int* flags, float* c, int size) {
    ...
    OccupancyMetrics occ = calculate_occupancy((const void*)segmented_scan_kernel, threadsPerBlock, 0);
